...
namespace tinylang
{
        ...
        if (auto Var = llvm::dyn_cast<VariableDeclaration>(D))
        {
            if (Var->getType() != E->getType())
            {
                Diags.report(Loc, diag::err_types_for_operator_not_compatible,
                             token::getPunctuatorSpelling(token::colonequal));
            }
    ...
    void Sema::actOnProcCall(StmtList &Stmts, SMLoc Loc, Decl *D, ExprList &Params)
    {
        if (auto Proc = llvm::dyn_cast<ProcedureDeclaration>(D))
        {
            ...
            if (Proc->getRetType()) Diags.report(Loc, diag::err_procedure_call_on_nonprocedure);
    ...
    void Sema::actOnReturnStatement(StmtList &Stmts, SMLoc Loc, Expr *RetVal)
    {
        ...
        {
            if (Proc->getRetType() != RetVal->getType()) Diags.report(Loc, diag::err_function_and_return_type);
        }
    ...
    Expr *Sema::actOnPrefixExpression(Expr *E, const OperatorInfo &Op)
    {
        ...
        if (E->isConst() && Op.getKind() == token::kw_not)
        {
        ...
        if (Op.getKind() == token::minus)
        {
            ...
            {
                token::TokenKind Kind = Infix->getOperatorInfo().getKind();
                if (Kind == token::star || Kind == token::slash) Ambiguous = false;
            }
            if (Ambiguous) { Diags.report(Op.getLocation(), diag::warn_ambigous_negation); }
        }
    ...
    Expr *Sema::actOnVariable(Decl *D)
    {
        ...
        {
            if (C == TrueConst) return TrueLiteral;
            if (C == FalseConst) { return FalseLiteral; }
    ...
    Expr *Sema::actOnFunctionCall(Decl *D, ExprList &Params)
    {
        ...
        if (auto *P = llvm::dyn_cast<ProcedureDeclaration>(D))
        {
            ...
            if (!P->getRetType()) Diags.report(D->getLocation(), diag::err_function_call_on_nonfunction);
    ...
    Decl *Sema::actOnQualIdentPart(Decl *Prev, SMLoc Loc, StringRef Name)
    {
        if (!Prev)
        {
            if (Decl *D = CurrentScope->lookup(Name)) return D;
        }
        else if (auto *Mod = llvm::dyn_cast<ModuleDeclaration>(Prev))
        {
